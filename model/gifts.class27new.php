...
function gatpackages()
	...
	//echo "sdfdsf";
	 global $callConfig;
	 $query=$callConfig->selectQuery(TPREFIX.TBL_PACKAGES,'*','','','','');
  ...
  function getGiftsData($id)
  {
	...
	$fieldnames=array('g_name'=>$name,'g_recepientname'=>$recepientname,'g_emailid'=>$emailid,'g_treatmenttype'=>$treatmenttype,'g_price'=>$price,'g_services'=>$treatment,'g_message'=>$message,'order_number'=>$orderid,'from_user'=>$from,'buyer_name'=>$buyersfullname,'buyer_phone'=>$buyerstelephone,'buyermessage'=>$other_message,'pickup_location'=>$location,'is_certified'=>$strcertified,'cretified_mesage'=>$certified_msg,'is_messanger'=>$strMessenger,'messager_msg'=>$Messenger_msg);
