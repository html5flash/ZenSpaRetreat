...
{ 
  ...
  function getOrderData($id)
  {
	...
	function insertOrder($post)
	{
		...
		{
		   $strNextDay=implode(",", $_POST['nextday']);
		}
		
	foreach($_POST as $key=>$val)  $$key=(get_magic_quotes_gpc())?$val:addslashes($val); 
		$fieldnames=array('order_number'=>$title,'order_packagename'=>$hdname,'order_price'=>$price,'order_otherprice'=>$total,'order_otherpackages'=>$strpackages,'fullname'=>$tofullname,'from_user'=>$from,'buyer_name'=>$buyersfullname,'buyer_phone'=>$buyerstelephone,'buyer_emailid'=>$buyersemailid,'buyermessage'=>$message,'pickup_location'=>$location,'is_certified'=>$strcertified,'cretified_mesage'=>$certified_msg,'is_messanger'=>$strMessenger,'messager_msg'=>$Messenger_msg, 'is_nextday'=>$strNextDay,'nextday_msg'=>$nextday_msg);
	...
	foreach($_POST as $key=>$val)  $$key=(get_magic_quotes_gpc())?$val:addslashes($val); 
		...
		$fieldnames=array('order_number'=>$orderid,'order_packagename'=>$hdname,'order_price'=>$_POST['price'],'order_otherprice'=>$total,'order_otherpackages'=>$strpackages,'fullname'=>$tofullname,'from_user'=>$from,'buyer_name'=>$buyersfullname,'buyer_phone'=>$buyerstelephone,'buyer_emailid'=>$buyersemailid,'buyermessage'=>$message,'pickup_location'=>$location,'is_certified'=>$strcertified,'cretified_mesage'=>$certified_msg,'is_messanger'=>$strMessenger,'messager_msg'=>$Messenger_msg, 'is_nextday'=>$strNextDay,'nextday_msg'=>$nextday_msg);
		//print_r($fieldnames);
