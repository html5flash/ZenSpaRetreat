...
{ 
  ...
  function getappointmentsData($id)
  {
	...
	$fieldnames=array('a_name'=>$name,'a_phone'=>$telephone,'a_emailid'=>$emailid,'a_celular'=>$celular,'a_treatment'=>$treatment,'a_treatmenttype'=>$treatmenttype,'a_addons'=>$otherpacks,'a_date'=>$app_date,'a_time'=>$hours.":".$mints,'order_number'=>$orderid,'a_package'=>$package,'a_pprice'=>$package_price);
...
function getpackid($id)
	{
	global $callConfig;
	$query=$callConfig->selectQuery(TPREFIX.TBL_PACKAGES,'package_one','package_id='.$id,'','','');
	return $callConfig->getRow($query);
	}
}
?>