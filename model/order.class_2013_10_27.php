...
{ 
  ...
  function getOrderData($id)
  {
	...
	foreach($_POST as $key=>$val)  $$key=(get_magic_quotes_gpc())?$val:addslashes($val); 
		...
		$fieldnames=array('order_number'=>$orderid,'order_packagename'=>$hdname,'order_price'=>$_POST['price'],'order_otherprice'=>$total,'order_otherpackages'=>$strpackages,'fullname'=>$tofullname,'from_user'=>$from,'buyer_name'=>$buyersfullname,'buyer_phone'=>$buyerstelephone,'buyer_emailid'=>$buyersemailid,'buyermessage'=>$message,'pickup_location'=>$location,'is_certified'=>$strcertified,'cretified_mesage'=>$certified_msg,'is_messanger'=>$strMessenger,'messager_msg'=>$Messenger_msg);
		//print_r($fieldnames);
