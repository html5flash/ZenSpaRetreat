...
class sitesettingsClass
{
 ...
 function lastlogin()
 {
  ...
  function sitesettings($post)
  {
	...
	$fieldnames=array('theme_selection'=>$post['theme_selection'],'title'=>mysql_real_escape_string($post['title']),'site_image'=>$logobanner,'noofrecords'=>$post['noofrecords'],'footer_txt'=>mysql_real_escape_string($post['footer_txt']),'googleanalytics'=>mysql_real_escape_string($post['googleanalytics']));
